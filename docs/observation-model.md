...
3. record a short parent-only memo, which is transcribed after the activity.
...
Because free text can accidentally contain PII, a future implementation must
warn the parent, avoid content logging, and discard transient inputs. Typed text
is screened before observation extraction. For an opt-in adult voice memo, raw
...
flowchart TD
  ...
  InputType -->|voice| Transcription["Transient adult-memo transcription"]
  InputType -->|typed| Screen["Transient PII screen or redact"]
  InputType -->|skip| End["End activity; no adaptive context"]
  Transcription --> Screen
  Screen --> Suggestion["Structured observation suggestion"]
...
For a future live OpenAI integration, do not rely on stored conversation or
thread state. Review the current endpoint’s storage behavior before deployment;
OpenAI documents default abuse-monitoring and application-state retention, as
well as available retention controls, in its [API data controls
documentation](https://platform.openai.com/docs/models/default-usage-policies-by-endpoint).
